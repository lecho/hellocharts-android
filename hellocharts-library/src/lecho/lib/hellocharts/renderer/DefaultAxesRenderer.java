...
public class DefaultAxesRenderer implements AxesRenderer {
	...
	private float[] axisXDrawBuffer = new float[] {};;
	...
	private void drawAxisHorizontal(Canvas canvas, Axis axis, float rawY, int position) {
		...
		for (AxisValue axisValue : axis.getValues()) {
			...
			if (value >= computator.getVisibleViewport().left && value <= computator.getVisibleViewport().right) {
				...
				if (axis.hasLines()) {
					axisXDrawBuffer[i * 4 + 0] = rawX;
					axisXDrawBuffer[i * 4 + 1] = computator.getContentRectWithMargins().top;
					axisXDrawBuffer[i * 4 + 2] = rawX;
					axisXDrawBuffer[i * 4 + 3] = computator.getContentRectWithMargins().bottom;
				}
			}
			++i;
		}
		...
		if (axis.hasLines()) {
			...
			canvas.drawLines(axisXDrawBuffer, 0, i, linePaint);
		}
	}
	...
	private void drawAxisHorizontalAuto(Canvas canvas, Axis axis, float rawY, int position) {
		...
		if (axis.hasLines() && axisXDrawBuffer.length < axisXStopsBuffer.numStops * 4) {
			axisXDrawBuffer = new float[axisXStopsBuffer.numStops * 4];
		}

		for (int i = 0; i < axisXStopsBuffer.numStops; ++i) {
			float rawX = computator.computeRawX(axisXStopsBuffer.stops[i]);
			final int nummChars = axis.getFormatter().formatValue(labelBuffer, axisXStopsBuffer.stops[i],
		...
		if (axis.hasLines()) {
			linePaint.setColor(axis.getLineColor());
			canvas.drawLines(axisXDrawBuffer, 0, axisXStopsBuffer.numStops * 4, linePaint);
		}
	}
