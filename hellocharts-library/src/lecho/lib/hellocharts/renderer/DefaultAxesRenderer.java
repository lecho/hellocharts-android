...
public class DefaultAxesRenderer implements AxesRenderer {
	...
	public void initAxesMeasurements() {
		...
		int axisXHeight = getAxisXBottomHeight(chart.getChartData().getAxisXBottom());
		int axisYWidth = getAxisYLeftWidth(chart.getChartData().getAxisYLeft(), chart.getChartData().getMaxAxisLabelChars());
	...
	private int getAxisXBottomHeight(Axis axisXBottom) {
		axesTextHeight = Math.abs(fontMetrics.ascent);
		int result = 0;

		if (axisXBottom.isAutoGenerated() || !axisXBottom.getValues().isEmpty()) {
			result += axesTextHeight;
		}

		if (!TextUtils.isEmpty(axisXBottom.getName())) {
			result += axesTextHeight;
		}
	...
	private int getAxisYLeftWidth(Axis axisYLeft, int maxLabelChars) {
		int result = 0;

		if (axisYLeft.isAutoGenerated()) {
			axesLabelMaxWidth = (int) textPaint.measureText(labelWidthChars, 0, maxLabelChars);
			result += axesLabelMaxWidth;
		} else if (!axisYLeft.getValues().isEmpty()) {
			final int numChars;
			// to simplify I assume that the widest value will be the first or the last.
			if (Math.abs(axisYLeft.getValues().get(0).getValue()) >= Math.abs(axisYLeft.getValues()
					.get(axisYLeft.getValues().size() - 1).getValue())) {
				numChars = axisYLeft.getFormatter().formatValue(labelBuffer, axisYLeft.getValues().get(0).getValue());
			} else {
				numChars = axisYLeft.getFormatter().formatValue(labelBuffer,
						axisYLeft.getValues().get(axisYLeft.getValues().size() - 1).getValue());
			}
			if (numChars > 0) {
		...
		if (!TextUtils.isEmpty(axisYLeft.getName())) {
			axesTextHeight = Math.abs(fontMetrics.ascent);
			result += axesTextHeight;
		}
	...
	private void drawAxisX(Canvas canvas) {
		...
		final Axis axisX = chart.getChartData().getAxisXBottom();
		...
		if (axisX.isAutoGenerated() || !axisX.getValues().isEmpty() || !TextUtils.isEmpty(axisX.getName())) {
			...
			canvas.drawLine(computator.getContentRectWithMargins().left, computator.getContentRect().bottom,
					computator.getContentRectWithMargins().right, computator.getContentRect().bottom, linePaint);
		}
	}
	...
	private void drawAxisY(Canvas canvas) {
		final ChartComputator computator = chart.getChartComputator();
		final Axis axisY = chart.getChartData().getAxisYLeft();
	...
	private void drawAxisY(Canvas canvas, ChartComputator computator, Axis axisY) {
	...
	private void drawAxisYAuto(Canvas canvas, ChartComputator computator, Axis axisY) {
