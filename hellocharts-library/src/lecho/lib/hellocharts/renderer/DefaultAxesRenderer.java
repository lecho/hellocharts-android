...
public class DefaultAxesRenderer implements AxesRenderer {
	...
	// Axis positions and also textPaints table indexes.
	private static final int TOP = 0;
	private static final int LEFT = 1;
	private static final int RIGHT = 2;
	private static final int BOTTOM = 3;
	// 4 text paints for every axis, not all have to be used.
	...
	public void initAxesMeasurements() {
		...
		int axisXTopHeight = initAxisMeasurementHorizontal(chart.getChartData().getAxisXTop(), TOP);
		int axisXBottomHeight = initAxisMeasurementHorizontal(chart.getChartData().getAxisXBottom(), BOTTOM);
	...
	/**
	 * Initialize measurement for horizontal axes(top,bottom); Returns axis measured height.
	 */
	private int initAxisMeasurementHorizontal(Axis axis, int position) {
		if (null == axis) {
			return 0;
		}
	...
	/**
	 * Initialize measurement for vertical axes(left, right); Returns axis measured width.
	 */
	private int initAxisMeasurementVertical(Axis axis, int position) {
		if (null == axis) {
			return 0;
		}
	...
	private void drawAxisX(Canvas canvas, int position) {
		...
		if (axisX.isAutoGenerated()) {
			drawAxisXAuto(canvas, computator, axisX, rawY, position);
		} else {
			drawAxisX(canvas, computator, axisX, rawY, position);
		}
		// Drawing axis name
		if (!TextUtils.isEmpty(axisX.getName())) {
			canvas.drawText(axisX.getName(), computator.getContentRect().centerX(), nameBaseline,
	...
	private void drawAxisX(Canvas canvas, ChartComputator computator, Axis axisX, float rawY, int position) {
	...
	private void drawAxisXAuto(Canvas canvas, ChartComputator computator, Axis axisX, float rawY, int position) {
		...
		for (int i = 0; i < axisXStopsBuffer.numStops; ++i) {
	...
	private void drawAxisVertical(Canvas canvas, int position) {
		...
		if (axisY.isAutoGenerated()) {
			drawAxisVerticalAuto(canvas, computator, axisY, rawX, position);
		} else {
			drawAxisVertical(canvas, computator, axisY, rawX, position);
		}
		...
		if (!TextUtils.isEmpty(axisY.getName())) {
			...
			canvas.drawText(axisY.getName(), computator.getContentRect().centerY(), nameBaseline,
					textPaintTab[position]);
			canvas.restore();
		}
	}

	private void drawAxisVertical(Canvas canvas, ChartComputator computator, Axis axis, float rawX, int position) {
		...
		for (AxisValue axisValue : axis.getValues()) {
			...
			if (value >= computator.getVisibleViewport().bottom && value <= computator.getVisibleViewport().top) {
				...
				if (axis.hasLines()) {
					axisYDrawBuffer[i * 4 + 0] = computator.getContentRectWithMargins().left;
	...
	private void drawAxisVerticalAuto(Canvas canvas, ChartComputator computator, Axis axis, float rawX, int position) {
		...
		if (axis.hasLines() && axisYDrawBuffer.length < axisYStopsBuffer.numStops * 4) {
			axisYDrawBuffer = new float[axisYStopsBuffer.numStops * 4];
		}

		for (int i = 0; i < axisYStopsBuffer.numStops; i++) {
			final float rawY = computator.computeRawY(axisYStopsBuffer.stops[i]);
			final int nummChars = axis.getFormatter().formatValue(labelBuffer, axisYStopsBuffer.stops[i],
