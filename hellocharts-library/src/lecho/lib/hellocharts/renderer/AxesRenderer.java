...
public class AxesRenderer {
	...
	private int[] axisStopsToDrawNumTab = new int[4];
	...
	private void drawAxisHorizontal(Canvas canvas, int position) {
		...
		if (axis.isAutoGenerated()) {
			drawAxisHorizontalAuto(canvas, axis, position);
		} else {
			drawAxisHorizontal(canvas, axis, position);
		}

	}

	private void drawAxisHorizontal(Canvas canvas, Axis axis, int position) {
		final ChartComputator computator = chart.getChartComputator();
		...
		axisStopsToDrawNumTab[position] = stopsToDrawIndex;

		if (axis.hasLines()) {
			drawAxisHorizontalLines(canvas, axis, contentRectMargins, position);
		}
		drawAxisHorizontalLabels(canvas, axis, contentRectMargins, position);

	}

	private void drawAxisHorizontalAuto(Canvas canvas, Axis axis, int position) {
		final ChartComputator computator = chart.getChartComputator();
		final Viewport visibleViewport = computator.getVisibleViewport();
		final Rect contentRect = computator.getContentRect();
		final Rect contentRectMargins = computator.getContentRectWithMargins();
	...
	private void drawAxisHorizontalLines(Canvas canvas, Axis axis, Rect contentRectMargins, int position) {

		int stopsToDrawIndex = 0;

		for (; stopsToDrawIndex < axisStopsToDrawNumTab[position]; ++stopsToDrawIndex) {
	...
	private void drawAxisHorizontalLabels(Canvas canvas, Axis axis, Rect contentRectMargins, int position) {
