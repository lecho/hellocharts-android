...
public class AxesRenderer {
	...
	private void prepareAxisToDraw(Axis axis, int position) {
		prepareAxisTextAlignment(axis, position);
	...
	private void prepareAxisTextAlignment(Axis axis, int position) {
		if (TOP == position || BOTTOM == position) {
		...
		for (AxisValue axisValue : axis.getValues()) {
			...
			if (value >= visibleViewport.left && value <= visibleViewport.right) {
				...
				if (0 == valueIndex % module) {
					...
					if (checkRawValue(contentRect, rawX, axis.isInside(), position, isAxisVertical)) {
	...
	private void prepareAutoGeneratedAxis(Axis axis, int position) {
		...
		FloatUtils.computeAutoGeneratedAxisValues(start, stop, contentRect.width() /
		...
		for (AxisValue axisValue : axis.getValues()) {
			...
			if (value >= visibleViewport.bottom && value <= visibleViewport.top) {
				...
				if (0 == valueIndex % module) {
					...
					if (checkRawValue(contentRect, rawY, axis.isInside(), position, isAxisVertical)) {
