...
public class SimpleAxisValueFormatter implements AxisValueFormatter {
	...
	public int formatValueForManualAxis(char[] formattedValue, AxisValue axisValue) {
		...
		//When there is no custom label format value.
		final int appliedDecimalDigitsNumber = getAppliedDecimalDigitsNumber(DEFAULT_DIGITS_NUMBER);
		final int charsNumber = formatFloatValue(formattedValue, axisValue.getValue(), appliedDecimalDigitsNumber);
	...
	public int formatValueForAutoGeneratedAxis(char[] formattedValue, float value, int autoDecimalDigits) {
		final int appliedDecimalDigitsNumber = getAppliedDecimalDigitsNumber(autoDecimalDigits);
		final int charsNumber = formatFloatValue(formattedValue, value, appliedDecimalDigitsNumber);
		appendText(formattedValue);
		prependText(formattedValue, charsNumber);
		return charsNumber + prependedText.length + appendedText.length;
	}

	private int getAppliedDecimalDigitsNumber(int defaultDigitsNumber){
		final int appliedDecimalDigitsNumber;
		if(this.decimalDigitsNumber >= 0){
			//When decimalDigitsNumber >=0 that means that user changed that value and defaultDigitsNumber should be overridden.
